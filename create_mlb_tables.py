#!/usr/bin/env python3
"""
Create MLB API tables in the sports_data database
"""
import psycopg2
import logging
...
def create_mlb_tables():
    """Create all MLB API related tables"""
    
    # Database connection (using the sports_data database)
    ...
    try:
        ...
        logger.info("Creating MLB tables...")
        ...
        cursor.execute("""
            ...
            AND table_name LIKE 'mlb_%'
            ORDER BY table_name;
        """)
        
        tables = cursor.fetchall()
        logger.info(f"✅ Created {len(tables)} MLB tables:")
        for table in tables:
...
if __name__ == "__main__":
    success = create_mlb_tables()
    if success:
        print("✅ MLB tables created successfully!")
    else:
        print("❌ Failed to create MLB tables")